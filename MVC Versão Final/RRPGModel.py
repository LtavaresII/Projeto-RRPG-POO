...
class MyEncoder(json.JSONEncoder):
	...
	def decode(d):
		...
		if d['tipo'] == 'Login':
			return Login(d['_Email'],d['_Senha'])
		if d['tipo'] == 'AtualizarSala':
			return AtualizarSala(d['_Sala'],d['_Usuario'])
		if d['tipo'] == 'AtualizarFicha':
			return AtualizarFicha(d['_Nome'],d['_Raca'],d['_Classe'],d['_Nivel'],d['_Vida'],d['_CA'],d['_Deslocamento'],d['_Antecedente'],d['_Forca'],d['_Destreza'],d['_Constituicao'],d['_Inteligencia'],d['_Sabedoria'],d['_Carisma'],d['_Equipamento'],d['_Ataques'],d['_Pericias'],d['_Testes'],d['_InformacaoPersonagem'])
					
class ErroChat(Exception):
	def __init__(self):
		super().__init__("Erro no Chat")
	
class ErroUsuarioIncorreto(Exception):
	def __init__(self):
		super().__init__("Usuário está errado ou não existe")
		
class ErroAtualizarFicha(Exception):
	def __init__(self):
		super().__init__("Erro ao atualizar ficha")
...
class Usuario:
	...
	def __repr__(self):
		return f'Usuario{self._PNome, self._SNome, self._Email, self._Senha, self._SenhaC}'
...
class Login:
	...
	def __repr__(self):
		return f'Usuario{self._Email, self._Senha}'
...
class AtualizarSala:
	def __init__(self, sala, usuario):
		
		self._Sala = sala
		self._Usuario = usuario
	
	def __repr__(self):
		return 'Atualizar({0},{1})'.format(self._Sala,self._Usuario)
